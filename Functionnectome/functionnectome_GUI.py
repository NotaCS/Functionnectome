...
class Functionnectome_GUI(tk.Tk):
    def __init__(self):
        ...
        self.fBOLD.grid(column=0, row=0, padx=epad, pady=epad, ipadx=ipad, ipady=ipad,
        ...
        self.buttonBold2 = tk.Button(self.fBOLD, text='Choose files',
        ...
        self.posSubName = tk.Spinbox(self.fBOLD, from_=0, to=0, width=5,
                                     textvariable=self.subInPath, command=self.update_exSub)
        ...
        self.ana_region = tk.Radiobutton(self.fAna, text="Region-wise functionnectome",
                                         variable=self.ana_type, value='region')
        self.ana_voxel = tk.Radiobutton(self.fAna, text="Voxel-wise functionnectome",
                                        variable=self.ana_type, value='voxel')
        ...
        self.parallel_proc = tk.Spinbox(self.fAna, from_=1, to=os.cpu_count(),
                                        width=5, textvariable=self.nb_parallel_proc)
        ...
        self.fAna.grid(column=2, row=0, padx=epad, pady=epad, ipadx=ipad, ipady=ipad,
                       columnspan=2, rowspan=1, sticky='news')
    ...
    def get_masks(self):
        ...
        def view_masks():
            if self.mask_paths_tmp:
                ...
                messagebox.showinfo("Selected masks", "No file selected yet.", parent=top_mask)

        def ok_maskBtn():
            if (nbmask.get() == 'common' and self.nbMasks.get() == 1
                    or nbmask.get() == 'subjectwise' and self.nbMasks.get() == self.nbFiles.get()):
        ...
        btn_paste = tk.Button(top_mask, text='Paste paths',
    ...
    def get_files_paste(self, paths_varname, nbpaths):
        ...
        def ok_btn():
            ...
            if paths_varname == 'bold_paths' and not lenPaths[1:] == lenPaths[:-1]:
                ...
                continue_answ = messagebox.askokcancel(title='Uneven path length', message=msg, parent=pasteWin)
    ...
    def saveSettings(self):
        ...
        if self.ana_type.get() == 'voxel' and not self.mask_paths:
            ...
            continue_answ = messagebox.askokcancel(title='No mask selected', message=msg, parent=self)
    ...
    def loadSettings(self):
        ...
        try:
            ...
            try:
                ...
                messagebox.showwarning('Bad value', 'The number of processes should be an integer', parent=self)
                return
